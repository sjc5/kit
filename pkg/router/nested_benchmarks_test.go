...
func setupBenchmarkPaths() *RouterBest {
	patterns := []string{
		"/$",
		"/_index",
		"/api/v1/users/_index",
...
func BenchmarkMatchCore(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		pathIdx := i % len(paths)
		router.SimpleMatch(patterns[patternIdx], paths[pathIdx], false, false)
	}
}
